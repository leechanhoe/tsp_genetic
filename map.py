...
def loadMap():
    ...
    cv2.putText(cityMap, org=(460, 575), text='Press Q to exit', fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(0, 0, 0), thickness=1, lineType=cv2.LINE_AA)
...
def updateUI(prevMap, generation, bestGene, time): # UI 갱신
    global cities
    ...
    # 아래 텍스트 갱신
    cv2.putText(cityMap, org=(20, 575), text='Generation: %d' % generation, fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(0, 0, 0), thickness=1, lineType=cv2.LINE_AA)
    cv2.putText(cityMap, org=(170, 575), text='Distance: %.2f' % bestGene.getFitness(), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(0, 0, 0), thickness=1, lineType=cv2.LINE_AA)
    cv2.putText(cityMap, org=(330, 575), text='Time: %d:%.1f' % (time//60, time%60), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(0, 0, 0), thickness=1, lineType=cv2.LINE_AA)
    cv2.imshow('cityMap', cityMap)
